...
export default function EnhancedSportsTable() {
  ...
  const fetchInternationalNews = async () => {
    try {
      const response = await axios.get('http://localhost:3001/api/sportsnews'); // Replace with your actual API endpoint
  ...
  const handleDeleteNews = async (title) => {
    try {
      await axios.delete('http://localhost:3001/api/deletesportsnews', { data: { title } }); // Replace with your actual delete API endpoint
      fetchInternationalNews();
    } catch (error) {
